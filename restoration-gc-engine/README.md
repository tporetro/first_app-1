...
3. **n8n — scripted where possible.** This session has no n8n MCP server/connector, so nothing here was run against a live n8n instance; run `03-n8n/setup_n8n.py` yourself against your own instance (needs `N8N_BASE_URL` + an n8n API key) to create the 4 secret-based credentials and import W1 in one shot. `RGC_HUBSPOT_PRIVATE_APP` is a single token, easiest pasted directly into n8n's HubSpot credential form. `RGC_GMAIL_OAUTH` and `RGC_LINKEDIN_OAUTH` can't be scripted at all — n8n's API can create the credential shell but not complete the OAuth consent redirect, so those two need a human in the n8n editor regardless of tooling. Build W2–W7 from their specs in `03-n8n/`; set webhook URLs; **keep W4 disabled.**
4. **Lead magnets — partially filled in, hosting still needed.** `__SUPABASE_URL__`, `__SUPABASE_ANON_KEY__`, and `__HS_PORTAL_ID__` are already replaced with real values in all 5 pages in `05-lead-magnets/`. Still outstanding, and not scriptable from this session (no tool here can create HubSpot Forms or Subscription Types, and there's no hosting connector attached):
