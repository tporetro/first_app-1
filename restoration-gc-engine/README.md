...
4. **Lead magnets — hosted, HubSpot form wiring still needed.** `__SUPABASE_URL__`, `__SUPABASE_ANON_KEY__`, and `__HS_PORTAL_ID__` are filled in with real values in all 5 pages. Fixed a real bug while preparing these for hosting: `subscriptionTypeId:__EMAIL_SUB_ID__` was an unquoted placeholder — a bare JS identifier, not a string — so the first real form submission would have thrown `ReferenceError` while building the HubSpot request body, aborting the whole submit handler *before* the Supabase writes ran (silently losing every lead and consent record, with no visible error). Quoted it as a string and wrapped the HubSpot call in try/catch so a bad/placeholder form GUID or subscription ID — or a HubSpot outage — can never block the Supabase writes, which are this system's actual source of truth.
   ...
   **Hosted and live** on GitHub Pages at `https://tporetro.github.io/first_app-1/` (e.g. `.../scorecard.html`) — Pages was already enabled on the `gh-pages` branch (root), no manual toggle was needed. If you want these on your own domain instead, point a custom domain there via GitHub's "Custom domain" field + your own DNS CNAME record; that step needs your DNS access, which isn't something any tool here has.

   **Functional testing of the live site (not just page-load) found and fixed 3 more bugs** — see risk register items #19–#20 for full detail:
   - `consent_records`'s two-object batch insert had mismatched JSON keys, so PostgREST rejected every single submission (`PGRST102`) — **zero consent rows had ever actually been captured** from any of the 5 live pages.
   - Even with matching keys, the DB's `chk_consent_disclosure` constraint would still reject the insert whenever the voice-consent checkbox was left unchecked (the default case), because `ai_voice_disclosed` was tied to the consent value instead of to whether the disclosure text was shown.
   - `hail_report_request.html` posted directly to `/rest/v1/properties` with the anon key, which has no INSERT/SELECT policy there (confirmed via RLS testing) — removed that insert rather than opening a new anon write surface; the address stays captured in `lead_magnet_submissions.answers`.

   All three fixed, verified end-to-end via curl against the live Supabase project, and pushed to both this branch and `gh-pages`. **Consent capture actually works now** — it did not before this pass, despite the pages rendering and submitting without any visible error.
...
5. **FastAPI — code and container image ready; actually provisioning a live host is still on you.** `10-fastapi/main.py` implements 4 endpoints (storm-alert, approve, retell-status, and the LinkedIn webhook handshake added in step 6) with HMAC verification; 18 tests in `10-fastapi/test_main.py` all pass. No hosting/deploy connector exists in this session, so an actual live URL can't be provisioned here — but everything short of that is done and verified:
   - **Ran the app as a real live process** (`uvicorn` + real HTTP requests, not just the test suite's in-process `TestClient`) and found a real bug the mocked tests missed: `/approve` and `/webhooks/retell-status` turned an HTTP error *status* from Supabase into a clean `502`, but a connection-level failure (Supabase unreachable) fell through uncaught as a raw `500` — bad especially for `/approve`, which a human opens directly from an email link. Fixed to match the error-handling pattern already used in `/webhooks/storm-alert`; verified live (500 → 502) before committing, plus 2 new regression tests. See risk register #25.
   - **Added a `Dockerfile` and `.dockerignore`** so deployment to any container host (Fly.io, Railway, a bare container) is copy-paste ready. Hand-reviewed but not build-tested — this sandbox has no Docker daemon available — so build and run it yourself before relying on it; the application code it wraps was independently verified as a real running process regardless of the container.

   Deploy it to any ASGI host — Railway, Fly.io, or `docker build -t rgc-fastapi . && docker run -p 8000:8000 --env-file .env rgc-fastapi` on your own container infrastructure. See `10-fastapi/README.md` for env vars and deploy notes.
6. **LinkedIn — everything scriptable is done; filing itself is a manual, LinkedIn-side step.** No LinkedIn API/developer-portal connector exists in this session, so the actual "create app → verify Page super-admin → apply for Development tier" sequence in `06-linkedin/community_api_application.md` has to be done by a human in LinkedIn's own UI — nothing here can do that for you. What *was* scriptable is done:
   - **Webhook validation handshake implemented and tested.** The application checklist required a working HMAC challenge-response endpoint before you can even register the webhook URL; it didn't exist. Added `GET /webhooks/linkedin` to `10-fastapi/main.py` (computes `hmac_sha256(LINKEDIN_CLIENT_SECRET, challengeCode)`, returns `{challengeCode, challengeResponse}`), covered by 2 new tests — 16/16 passing.
   - **Privacy-policy prerequisite verified.** The application requires a live privacy policy URL. Checked `restorationgc.net/privacy` directly: it's real (redirects to `/privacy-policy/`, a substantive WordPress page covering PII/cookies/CCPA/GDPR), not a placeholder. **Gap found:** it doesn't disclose what the Community Management API integration does with data (Page analytics, OAuth tokens, comment/reaction data) — LinkedIn's terms expect that disclosure. This needs a manual edit to the live WordPress site (no CMS tool available here) before filing.
   - Publish the profile/Company Page rewrite (`06-linkedin/profile_rewrite.md`) — also manual, since it's editing MJ's own LinkedIn identity, not something any tool should do unattended even if one existed.
7. **Content — done and re-verified.** All 30 rows from `07-content/30_day_evergreen.md` are loaded into `content_drafts` in the live `restoration-gc-engine` Supabase project (`mode = 'evergreen'`, `compliance_status = 'generated'`, scheduled sequentially via `scheduled_date` starting today) via `01-supabase/008_content_calendar_seed.sql`. That migration also added `content_drafts.scheduled_date` (not in the original schema) since nothing told W3 which calendar row is "today's slot" without it — W3's spec was updated to select `where scheduled_date = current_date`. Loading the calendar again elsewhere: run `008_content_calendar_seed.sql` once against a fresh project (it has no dedupe key, so re-running it duplicates rows — check the table is empty first). Re-confirmed live: 30/30 rows present, 30 distinct scheduled dates, today's slot exists.

   **Checking the loaded rows against the actual compliance lexicon (not just trusting them) found and fixed a real bug.** These rows are content *briefs* (e.g. "IL 215 ILCS 5/155.51 deductible-fraud statute explained"), not final copy, so the deterministic regex checker doesn't apply to them directly — but running it anyway surfaced that `il_no_deductible` had no first-person/offer scoping, unlike its TX/OK counterparts: it would have blocked the compliant educational content Day 22's brief itself calls for, forcing the gate to reject its own required output. Fixed live and folded into `01-supabase/006_seed.sql`; see `08-testing/compliance_redteam.md` (now 46/46 fixtures) and risk register #22. A second bug, in the test harness itself — `run_redteam.py` silently reported a false "all clean" result when run with a key that can't read `compliance_rules` — is fixed in the same pass (risk register #23).
8. **Counsel review — approved a version that has since changed; a fresh, targeted confirmation is needed before first send.** Counsel reviewed and approved the compliance lexicon and disclaimers. Since that approval, the lexicon has been corrected 6 times total (5 in the original red-team pass, 1 more found while verifying the loaded content calendar in this pass) — all found by actually running the rules against real text, not by reading them. **No disclaimer text, citation, or banned category changed**; every fix either catches more of what was already banned or stops the gate from blocking its own compliant required output. One item (`tx_contract_notice`'s scope) is a procedural clarification, and one (`il_no_deductible`'s subject-scoping, done in this pass) is a genuine narrowing of automated coverage. `09-docs/counsel_review_delta.md` is the exact diff, written so counsel can confirm it in minutes rather than re-reviewing from scratch — send that document before treating this step as done again. Obtaining that actual confirmation is on MJ; no tool here can stand in for real legal review.
9. **Enable W4 — both gates cleared, re-verified against everything that's changed since, activation is on you.** The red-team run is genuinely clean: `08-testing/run_redteam.py`, run against the live `compliance_rules` table (not just read over), found and fixed 6 real bugs total (see `08-testing/compliance_redteam.md`) — current result **46/46 fixtures match expected status, 0 `block`-severity items marked `pass`.** MJ has signed off on the Retell consent flow, and nothing touching that flow's code has changed since. **Given how much schema and infrastructure has shifted underneath W4 since it was written** (new columns, RLS fixes, the FastAPI service's own bug fixes), its own SQL and gating logic were re-verified directly rather than assumed still correct: all 4 of W4's embedded queries (`Fetch Pending Dispatch`, `Fetch Latest Consent`, `Mark Dispatched`, `Log Attribution Event`) were run through `EXPLAIN` against the live schema and all plan cleanly — no drift. The `Compute Dispatch Gate` Code node's logic (voice requires `consent_given AND ai_voice_disclosed`; email requires just `consent_given`) still matches both the `chk_consent_disclosure` DB constraint and the corrected `ai_voice_disclosed` semantics now live in all 5 lead-magnet pages. With both gates cleared and re-verified, W4 (`03-n8n/W4_consent_gated_dispatch.json`, importable via `setup_n8n.py`) is ready — but it imports **inactive** by design, and activating it (n8n editor → open the workflow → toggle Active) is a manual step this build deliberately never automates, since nothing in code can substitute for a human's own decision to flip on real outbound dispatch. No tool in this session could do it even if that principle didn't hold — there is no n8n connector here at all.
...
1. **Now:** run the Supabase migration; file the LinkedIn Community Management API application (longest lead time); send the lexicon to counsel — and if this is a re-send after the initial approval, send `09-docs/counsel_review_delta.md` instead of the raw lexicon so counsel is confirming the actual diff, not re-reading everything. *Threshold to proceed:* PostGIS + RLS verified; counsel sign-off received against the **current** lexicon, not a prior version of it.
