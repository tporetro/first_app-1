...
16 tests cover: HMAC verification (missing/invalid/valid signature) on both webhook endpoints, payload validation, forwarding to n8n plus the downstream-unreachable case, `/approve`'s unknown-token/success/already-decided/bad-decision-value paths, the retell-status consent/disclosure gate, and the LinkedIn challenge-handshake response (correct HMAC + missing-param validation). All were run against this exact code before it was committed — not just read over.
